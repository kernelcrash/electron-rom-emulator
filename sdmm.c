...
// Start off with a slow clock rate. Set to 0 later to switch to faster SPI
static
BYTE	BitDelay = 3;
...
)
{
	...
	if (!BitDelay) {
		...
		do {
			r = 0;	 if (DO) r++;	/* bit7 */
			CK_H(); dly_us(BitDelay); CK_L(); dly_us(BitDelay);
			r <<= 1; if (DO) r++;	/* bit6 */
			CK_H(); dly_us(BitDelay); CK_L(); dly_us(BitDelay);
			r <<= 1; if (DO) r++;	/* bit5 */
			CK_H(); dly_us(BitDelay); CK_L(); dly_us(BitDelay);
...
static
int sdmm_select (void)	/* 1:OK, 0:Timeout */
{
	BYTE d;
...
)
{
	...
	dly_us(10000);			/* 10ms */
	// assume GPIO is already initialised
